...
if (cp != NULL)
  ...
  if (nerrorcp == OK)
    {
    nerrorcp = Stack_ok(&(cp->CallStack));
    if (nerrorcp == OK)
      {
...
assert(dump != NULL);
fprintf(dump,"================================================================================\n");
fprintf(dump,"The list of ERRORS:\n");
fprintf(dump,"OK        = 0:                   stack is normal\n");
fprintf(dump,"BAD_PUSH  = 1:                   stack has elements in zero cell\n");
fprintf(dump,"BAD_NTOP  = 2:                   the number of the last element in stack is more\n");
fprintf(dump,"                                 than the maximum allowable number or it's less\n");
fprintf(dump,"                                 than zero\n");
fprintf(dump,"STRERROR  = 3:                   NULL pointer on the body of stack\n");
fprintf(dump,"BAD_INPUT = 4:                   bad parameters were transmitted to function\n");
fprintf(dump,"NULL_POINTER_ON_STACK = 5:       null pointer on structure\n");
fprintf(dump,"NULL_POINTER_ON_REGISTER_ax = 6: null pointer on register ax\n");
fprintf(dump,"NULL_POINTER_ON_REGISTER_bx = 7: null pointer on register bx\n");
fprintf(dump,"NULL_POINTER_ON_REGISTER_cx = 8: null pointer on register cx\n");
fprintf(dump,"NULL_POINTER_ON_REGISTER_dx = 9: null pointer on register dx\n");
fprintf(dump,"NULL_POINTER_ON_CPU = 10:        null pointer on cpu\n");
fprintf(dump,"BAD_ZERO_CELL = 11:              there is not a POISON = %d in zero cell\n", POISON);
fprintf(dump,"--------------------------------------------------------------------------------\n");
ERRORS nerrorcp = CPU_ok(cp);
fprintf(dump,"EDCPU_t (%d) [%p]\n", nerrorcp, cp);
if (nerrorcp != NULL_POINTER_ON_CPU)
  {
  if (nerrorcp != NULL_POINTER_ON_REGISTER_ax) fprintf(dump,"Register ax = %lg\n", cp->ax);
  if (nerrorcp != NULL_POINTER_ON_REGISTER_bx) fprintf(dump,"Register bx = %lg\n", cp->bx);
  if (nerrorcp != NULL_POINTER_ON_REGISTER_cx) fprintf(dump,"Register cx = %lg\n", cp->cx);
  if (nerrorcp != NULL_POINTER_ON_REGISTER_dx) fprintf(dump,"Register dx = %lg\n", cp->dx);
